...
import pandas as pd
import matplotlib
matplotlib.use("Agg")  # ✅ Evita errores en entornos sin interfaz gráfica
...
# === CARGA DE DATASET NASA (columna corregida pl_disposition) ===
def cargar_dataset():
    nasa_url = (
        "https://exoplanetarchive.ipac.caltech.edu/TAP/sync?"
        "query=select+top+2000+pl_name,disc_year,discoverymethod,"
        "pl_rade,pl_bmasse,default_flag,pl_disposition+from+ps&format=csv"
    )
    try:
...
def train_model():
    ...
    # 🔧 Asegurar compatibilidad de columna
    if "pl_disposition" in df.columns:
        df.rename(columns={"pl_disposition": "disposition"}, inplace=True)
    ...
    # Etiquetas (Confirmed, Candidate, False Positive)
